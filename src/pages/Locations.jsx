...
const Locations = () => {
      ...
      try {
        ...
        const locationsWithImages = await Promise.all(
          response.data.map(async (location) => {
            try {
              // NOTE: This call fetches location details including images.
              // For better performance, consider if the /api/locations endpoint 
              // could be updated to include the main image URL directly.
  ...
  const getMainImageUrl = (location) => {
    ...
    if (mainImage) {
      // Construct full URL for the main image
      return `${API_BASE_URL}${mainImage.url}`;
    }

    // If no main image found, return the first image
    return `${API_BASE_URL}${location.images[0].url}`;
  };
